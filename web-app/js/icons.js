/**
 * Icones SVG - Application de Pointage RH
 * Icones simples avec couleur uniforme (currentColor)
 */
...
const ICONS = {
    ...
    userX: `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M16 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"></path><circle cx="8.5" cy="7" r="4"></circle><line x1="18" y1="8" x2="23" y2="13"></line><line x1="23" y1="8" x2="18" y2="13"></line></svg>`,

    clockFilled: `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><polyline points="12 6 12 12 16 14"></polyline></svg>`,
...
/**
 * Fonction utilitaire pour obtenir une icone
 * @param {string} name - Nom de l'icone
 * @param {string} size - Taille optionnelle (ex: '24')
 * @returns {string} - SVG de l'icone
 */
function getIcon(name, size = null) {
    let icon = ICONS[name] || '';
    if (size && icon) {
        icon = icon.replace(/width="[^"]*"/, `width="${size}"`);
        icon = icon.replace(/height="[^"]*"/, `height="${size}"`);
    }
    return icon;
}
